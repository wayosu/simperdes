                        ...
                        <div class="card-header">
                            @if (Auth::user()->role == 'admin_desakel')
                                <div class="d-flex align-items-center justify-content-between">
                                    ...
                                    <a href="{{ route('admin-desakel.perdes.create') }}"
                                        class="btn btn-sm btn-primary"><i class="fas fa-upload"></i> Tambah Data</a>
                                </div>
                            @else
                                <h3 class="card-title">{{ $title }}</h3>
                            @endif
                                    ...
                                    <tbody>
                                        @foreach ($datas as $result)
                                            <tr>
                                                ...
                                                <td>
                                                    <div class="d-flex">
                                                        <a href="{{ route('admin-desakel.perdes.detail', $result->id) }}" class="btn btn-sm text-white btn-info mr-2">
                                                            <i class="fas fa-info-circle"></i> Detail
                                                        </a>
                                                        <a href="{{ route('admin-desakel.perdes.edit', $result->id) }}" class="btn btn-sm text-white btn-warning mr-2">
                                                            <i class="fas fa-edit"></i> Edit
                                                        </a>
                                                        <form method="POST" action="{{ route('admin-desakel.perdes.destroy', $result->id) }}">
                                                            @csrf
