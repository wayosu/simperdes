                    ...
                    <div class="card">
                        <div class="card-header">
                            <div class="d-flex align-items-center">
                                <a href="{{ route('admin-desakel.peraturan-desa') }}" class="text-secondary mr-2"><i class="fas fa-arrow-alt-circle-left"></i></a>
                                <h3 class="card-title">Form {{ $subtitle }}</h3>
                            </div>
                        </div>
                        <div class="card-body">
                            <form action="{{ route('admin-desakel.peraturan-desa.update', $data->id) }}" method="POST" enctype="multipart/form-data">
                                @csrf
                                @method('put')

                                <div class="form-group">
                                    <label for="judul_peraturan">Judul Peraturan <span class="text-danger">*</span></label>
                                    <input type="text" name="judul_peraturan" id="judul_peraturan" class="form-control @error('judul_peraturan') is-invalid @enderror" value="{{ old('judul_peraturan', $data->judul_peraturan) }}" placeholder="...">
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="jenis_peraturan_id" id="jenis_peraturan_id" class="form-control select2 @error('jenis_peraturan_id') is-invalid @enderror" style="width: 100%;">
                                        ...
                                        @foreach ($jenis_peraturans as $result)
                                            <option value="{{ $result->id }}" @if ($data->jenis_peraturan_id == $result->id) selected @endif>{{ $result->jenis_aturan }}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea name="isi_peraturan" id="isi_peraturan" class="form-control @error('isi_peraturan') is-invalid @enderror">{{ $data->isi_peraturan }}</textarea>
                                    @error('isi_peraturan')
                                        <span class="error invalid-feedback">
                                            {{ $message }}
                                        </span>
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" name="nama_penyusun" id="nama_penyusun" class="form-control @error('nama_penyusun') is-invalid @enderror" value="{{ old('nama_penyusun', $data->nama_penyusun) }}" placeholder="...">
                                    @error('nama_penyusun')
                                        <span class="error invalid-feedback">
                                            {{ $message }}
                                        </span>
